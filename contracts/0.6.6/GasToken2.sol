...
contract GasToken2 {
    ...
    uint256 ADDRESS_BYTES = 20;
    uint256 MAX_SINGLE_BYTE = 128;
    uint256 MAX_NONCE = 256**9 - 1;

    // count number of bytes required to represent an unsigned integer
    function count_bytes(uint256 n) view internal returns (uint256 c) {
        uint i = 0;
        uint mask = 1;
        while (n >= mask) {
            i += 1;
    ...
    function mk_contract_address(address a, uint256 n) view internal returns (address rlp) {
        /*
    ...
    string public name = "Gastoken.io";
    string public symbol = "GST2";
